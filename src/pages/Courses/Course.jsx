                            ...
                            <>
                                {
                                    information.courses.some((element) => {
                                            ...
                                            <p>Tình trạng:
                                                {role === 'student' ?
                                                    <>
                                                        đang học
                                                        {/* <div></div> */}
                                                        <Button type="primary" onClick={() => deleteStudent(`users/${auth.currentUser?.uid}`, `courses`, `/courses/${courseEle.id}`,
                                                            `courses/${courseEle.id}`, 'students', `users/${auth.currentUser?.uid}`)}>Hủy môn học</Button>
                                                    </> :
                                                    (role === 'teacher' ?
                                                        <>
                                                            Đang dạy
                                                            {/* <div></div> */}
                                                            <Button type="primary" onClick={() => deleteTeacher(`users/${auth.currentUser?.uid}`, `courses`, `/courses/${courseEle.id}`,
                                                                `courses/${courseEle.id}`, 'teacher')}>Hủy dạy</Button>
                                                        </> :
                                                        <>role: admin</>
                                                    )
                                                }
                                        ...
                                        <>
                                            {role === 'student' ?
                                                <>
                                                    <Button type="primary" onClick={() => handleSubmit(`users/${auth.currentUser?.uid}`, 'courses', `/courses/${courseEle.id}`, courseEle.id, role)} >Đăng ký học</Button>
                                                </>
                                                :
                                                (role === 'teacher' ?
                                                    <>
                                                        {'teacher' in courseEle ? <> Đã có giáo viên giảng dạy</> :
                                                            <>
                                                                <Button type="primary" onClick={() => handleSubmit(`users/${auth.currentUser?.uid}`, 'courses', `/courses/${courseEle.id}`, courseEle.id, role)}>Đăng ký dạy</Button>
                                                            </>
                                                        }
                                                    </> :
                                                    <>admin</>
